...
- GitHub: [tannyann/cosmic-id](https://github.com/tannyann/cosmic-id)（`lp/` を push 済みであること）
- Vercel アカウント
- Supabase プロジェクト
...
`lp/.env.local`（ローカル）および Vercel Dashboard → Settings → Environment Variables:
...
1. `lp/` を `main` に push
2. [vercel.com/new](https://vercel.com/new) → Import `tannyann/cosmic-id`
3. **Root Directory** = `lp`
4. Environment Variables を Step 2 の通り設定
5. Deploy
