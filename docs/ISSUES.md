...
### I-1 流派・方式の採否決定(5論点)【担当: 人間】

**背景**: 監査(`docs/audit/fortune-accuracy.md` §6、`docs/audit/parts/others.md`)で、経営判断が必要な論点が出揃った。以降の実装 Issue の多くがこの決定待ち。

**決めること(それぞれ選択肢とおすすめは監査レポートに記載済み)**:
1. マヤ暦KIN: I-2 の照合が黒なら公式準拠に修正するか、「独自簡易KIN」明記で維持か
2. タロット22: 愚者(0)にマップするか(おすすめ)、別还元か
3. lifePath: 全桁合計(現状)か個別還元(主流)か併記か — マスターナンバーの出方が変わる
4. 年の切替: 干支・五行を九星と同じ立春系に統一するか(おすすめ)、1/1維持+注記か
5. 動物占い: 「独自12分類」明記(おすすめ・即日)か、公式互換化(法務確認要)か
